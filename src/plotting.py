...
def barplot_multiple_dataframes(dataframes, labels, title, xlabel, ylabel, legend=True):
    ...
    for i in range(len(dataframes)):
        plt.bar(bar_positions[i], dataframes[i],
...
def boxplot(list_of_lists, labels, title, xlabel, ylabel):
    ...
    box = ax.boxplot(list_of_lists, positions=positions, widths=0.6,
...
def bar_plot_many_datapoints(
    ...
    bar_plot = sns.barplot(x=x_column, y=y_column,
                           data=df, hue='leaning', dodge=False)
